    ...
    cx.render(rsx! {
        div { class: "flex ml-2 mr-2 items-center",
            ...
            button {
                ...
                onclick: move |_| {
                    if let Ok(Some(path))
                        = FileDialog::new().add_filter("Image (png)", &["png"]).show_open_single_file()
                    {
                        let file_name = path
                            .file_name()
                            .expect("Is a file")
                            .to_str()
                            .expect("Is valid unicode");
                        let new_item = BinItem::new_from_file(&file_name, &path.to_string_lossy());
                        match new_item {
                            Ok(bin_item) => {
                                let mut atom_instance = atom_instance.write();
                                let mut_instance = atom_instance
                                    .as_mut()
                                    .expect("DCE instance is loaded");
                                if let Some(existing_item)
                                    = mut_instance
                                        .bin_data
                                        .images
                                        .iter_mut()
                                        .find(|i| i.name.as_str() == file_name)
                                {
                                    log::trace!(
                                        "Image already exists with name {}, replacing data", file_name
                                    );
